...
const handleExport = async (startDate, endDate) => {
  ...
  const response = await fetch(`${apiBaseUrl}/export/excel?${params}`, {
    headers: { Accept: "..." },
  });
};
```
...
if (measurements.length > 0) {
  const avgFill =
    measurements.reduce((sum, m) => sum + m.fill_level_percent, 0) /
    measurements.length;
...
const mostActive = Object.entries(binCounts).sort(([, a], [, b]) => b - a)[0];
...
```javascript
const measurements = await queryDatabase(
  db,
