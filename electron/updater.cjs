...
const PENDING_FLAG = path.join(
  app.getPath("userData"),
  "pending-installer.json",
);
...
function downloadFile(url, destPath) {
  return new Promise((resolve, reject) => {
    const follow = (u) => {
      https
        .get(u, { headers: { "User-Agent": "BinThere-Updater" } }, (res) => {
          if (res.statusCode === 301 || res.statusCode === 302) {
...
function applyPendingInstall() {
  ...
  if (!installerPath || !fs.existsSync(installerPath)) {
    console.log(
      "[Updater] Pending installer not found on disk — clearing flag.",
    );
    fs.rmSync(PENDING_FLAG, { force: true });
    return;
  }
  ...
  console.log(
    `[Updater] Applying pending update to v${version} — running installer silently…`,
  );
...
async function silentUpdateCheck() {
  try {
    ...
    if (!asset) {
      console.warn(
        "[Updater] Newer release found but no .exe asset — skipping.",
      );
      return;
    }

    console.log(
      `[Updater] v${remoteVersion} available. Downloading installer…`,
    );

    const tmpDir = path.join(
      app.getPath("temp"),
      `binthere-update-${Date.now()}`,
    );
    ...
    console.log(
      `[Updater] Installer ready at ${installerPath}. Will apply on next restart.`,
    );
  } catch (e) {
