...
function getFillState(binId, compartment) {
  ...
  if (!fillStateCache.has(key)) {
    const last = db
      .prepare(
        `SELECT fill_level_percent FROM measurements WHERE bin_id=? AND compartment=? ORDER BY timestamp DESC LIMIT 1`,
      )
...
function getBinWithCompartments(binId) {
  ...
  const latestDry = db
    .prepare(
      `SELECT * FROM measurements WHERE bin_id = ? AND compartment = 'dry' ORDER BY timestamp DESC LIMIT 1`,
    )
    .get(binId);
  const latestWet = db
    .prepare(
      `SELECT * FROM measurements WHERE bin_id = ? AND compartment = 'wet' ORDER BY timestamp DESC LIMIT 1`,
    )
    .get(binId);
...
const LOG_ROUTES = [
  {
    ...
    message: "Security: Authorized administrative session established",
  },
  {
    pattern: /^\/api\/auth\/register$/,
    message:
      "Access Control: Provisioned credentials for a new system operator",
  },
  {
    pattern: /^\/api\/auth\/me$/,
    message: "Identity Verification: Confirmed active session credentials",
  },
  {
    pattern: /^\/api\/bins$/,
    method: "GET",
    message:
      "Fleet Oversight: Retrieved real-time status for all deployed assets",
  },
  {
    pattern: /^\/api\/bins$/,
    method: "POST",
    message:
      "Infrastructure Expansion: Integrated a new unit into the active service fleet",
  },
  {
    pattern: /^\/api\/bins\/\d+$/,
    method: "DELETE",
    message:
      "Fleet Maintenance: Retired a service unit from the active registry",
  },
  {
    pattern: /^\/api\/bins\/\d+$/,
    method: "PATCH",
    message:
      "Optimization: Calibrated configuration settings for a specific asset",
  },
  {
    pattern: /^\/api\/analytics\/utilization$/,
    message:
      "Efficiency Audit: Evaluated current resource load across the network",
  },
  {
    pattern: /^\/api\/analytics\/fleet-history$/,
    message:
      "Performance Review: Compiled historical trends for the 7-day operational cycle",
  },
  {
    pattern: /^\/api\/analytics\/fleet-fill-cycles(\?.*)?$/,
    message: "Fleet Cycles: Aggregated total fill cycles across all bins",
  },
  {
    pattern: /^\/api\/bins\/\d+\/analytics$/,
    message:
      "Unit Analysis: Benchmarked efficiency metrics for a specific asset",
  },
  {
    pattern: /^\/api\/bins\/\d+\/heatmap$/,
    message:
      "Strategic Mapping: Visualized peak activity patterns and usage density",
  },
  {
    pattern: /^\/api\/bins\/\d+$/,
    method: "GET",
    message:
      "Data Inspection: Reviewed comprehensive historical logs for a specific unit",
  },
  {
    pattern: /^\/api\/bins\/\d+\/measurement$/,
    message:
      "Diagnostics: Validated real-time telemetry flow from remote hardware",
  },
  {
    pattern: /^\/api\/sensor-data$/,
    message:
      "System Feed: Processed automated data stream from the sensor network",
  },
  {
    pattern: /^\/api\/export\/excel(\?.*)?$/,
    message: "Reporting: Generated standardized audit document (Excel format)",
  },
  {
    pattern: /^\/api\/export\/metadata$/,
    message:
      "System Integrity: Verified availability of archival data for reporting",
  },
  {
    pattern: /^\/api\/logs\/event$/,
    message:
      "Audit Trail: Logged manual administrative interaction with the dashboard",
  },
  {
    pattern: /^\/api\/config\/status$/,
    message: "System Configuration: Retrieved dynamic threshold parameters",
  },
  {
    pattern: /^\/api\/config\/save$/,
    message:
      "System Configuration: Modified threshold variables and updated operational environment",
  },
  {
    pattern: /^\/api\/admin\/backup$/,
    message: "System Maintenance: Initiated database snapshot backup",
  },
];

const getHumanReadableLog = (method, url, statusCode) => {
  const match = LOG_ROUTES.find(
    (r) => r.pattern.test(url) && (!r.method || r.method === method),
  );
  ...
  return match
    ? `${statusContext} | ${match.message}`
    : `${statusContext} | Activity: ${method} ${url}`;
};
...
app.use((req, res, next) => {
  res.on("finish", () => {
    ...
    const action = getHumanReadableLog(
      req.method,
      req.originalUrl,
      res.statusCode,
    );
...
app.post("/api/auth/register", async (req, res) => {
  ...
  res
    .status(201)
    .json({ status: "success", token: signToken(newUser), user: newUser });
});
...
wss.on("connection", (ws) => {
  ...
  getAllBinsWithCompartments().forEach((bin) =>
    ws.send(JSON.stringify({ type: "state", bin })),
  );
...
app.get("/api/analytics/utilization", requireAuth, (req, res) => {
  const row = db
    .prepare(
      `SELECT AVG(fill_level_percent) as avgFill FROM measurements WHERE timestamp >= datetime('now', '-24 hours')`,
    )
    .get();
  res.json({
    status: "success",
    utilization_score: row?.avgFill != null ? Math.round(row.avgFill) : 0,
  });
});
...
app.get("/api/analytics/fleet-history", requireAuth, (req, res) => {
  ...
  const dayMap = {};
  rows.forEach((r) => {
    dayMap[r.day] = Math.round(r.avgFill);
  });
...
app.get("/api/bins/:id/analytics", requireAuth, (req, res) => {
  ...
  const rows = db
    .prepare(
      `SELECT date(filled_at) AS day, compartment, COUNT(*) AS cnt FROM fill_cycles WHERE bin_id = ? AND filled_at >= datetime('now', ? || ' days') GROUP BY day, compartment`,
    )
    .all(binId, `-${days}`);
...
app.post("/api/bins/:id/measurement", requireAuth, (req, res) => {
  ...
  db.prepare(
    `INSERT INTO measurements (bin_id, compartment, raw_distance_cm, fill_level_percent, timestamp) VALUES (?, ?, ?, ?, ?)`,
  ...
  broadcast({
    type: "update",
    bin: cachedBin || getBinWithCompartments(binId),
  });
...
app.post("/api/admin/backup", requireAdmin, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ status: "error", message: `Backup failed: ${err.message}` });
  }
});
...
app.get("/api/admin/select-directory", requireAdmin, (req, res) => {
  if (process.platform !== "win32") {
    return res.status(400).json({
      status: "error",
      message:
        "Native directory selection is only supported on Windows host systems.",
    });
  }
...
function purgeOldData() {
  ...
  function purgeBatch() {
    ...
    if (deleted > 0) {
      ...
      if (totalDeleted > 0) {
        console.log(
          `🧹 Purge complete — ${totalDeleted} rows removed, running VACUUM`,
        );
        db.exec("VACUUM");
        rebuildFleetCache();
      } else {
